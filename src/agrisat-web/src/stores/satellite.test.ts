...
function makeTLE(id: string): TLERecord {
	return {
		...
		line2:
			"2 25544  51.6400 208.9163 0006703 300.0000  60.0000 15.50000000000011",
...
describe("Satellite Store", () => {
	...
	describe("setSatelliteTLEs", () => {
		...
		it("should clear selection when selected satellite TLE is removed (Req 7.5)", () => {
			...
			useSatelliteStore
				.getState()
				.setSatelliteTLEs([makeTLE("SAT-1"), makeTLE("SAT-2")]);
			useSatelliteStore.getState().setSelectedId("SAT-1");
			useSatelliteStore
				.getState()
				.setGroundTrack([makeGroundTrackPoint(10, 20)]);
		...
		it("should keep selection when selected satellite TLE is still present", () => {
			...
			useSatelliteStore
				.getState()
				.setSatelliteTLEs([makeTLE("SAT-1"), makeTLE("SAT-3")]);
		...
		it("should trim positions to only include satellites in new TLE set (Req 7.2)", () => {
			...
			useSatelliteStore
				.getState()
				.setSatellitePositions([makePosition("SAT-1"), makePosition("SAT-2")]);
